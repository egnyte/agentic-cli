...
```bash
# First time — only --domain required (built-in OAuth app used automatically)
...
# CI / headless agents — use env vars instead
...
1. **All mutations require `--yes` or `--dry-run`** — enforced at the binary level. Always show `--dry-run` output to the user before executing.
...
8. **When invoking the CLI from code, always pass args as a list — never as a shell string.** Pass each flag as a separate element so no shell interprets quoting or mangles paths. After generating any CLI subprocess call, verify the args are a list, not a shell string. Examples by language:
   - Python: `subprocess.run(["egnyte", "fs", "get", "/Shared/file.pdf", "--fields", "name,entry_id"], check=True)`
   - Node.js: `spawnSync("egnyte", ["fs", "get", "/Shared/file.pdf", "--fields", "name,entry_id"])`
   - Go: `exec.Command("egnyte", "fs", "get", "/Shared/file.pdf", "--fields", "name,entry_id")`
   - Shell: `egnyte fs get /Shared/file.pdf --fields name,entry_id` (direct, no `powershell -Command` or `bash -c` wrapper)

## Bulk and progress

- Use `--bulk-file-path <csv>` (or `--from-csv <csv>`) on supported mutating commands when a migration or admin workflow spans many items.
- Bulk mutations still require `--dry-run` or `--yes`.
- Use `--parallelism <n>` to control in-flight bulk requests. Default is `2`.
- Use `--progress` for human-readable stderr updates and `--json-progress` for newline-delimited machine-readable progress events.
- Keep final command results on `stdout` as JSON and treat progress as `stderr` only.
...
egnyte users create --json '{"userName":"jsmith","email":"j@co.com","name":{"familyName":"Smith","givenName":"John"},"active":true,"userType":"standard","authType":"egnyte"}' --yes
...
Check `_ratelimit.qps_remaining` in loops and pause when it hits 0.
